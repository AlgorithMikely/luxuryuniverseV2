...
async def get_line_view(
    ...
    # 9. Dynamic Pricing Logic (Removed to respect reviewer settings)
    # surge_price = 3.99
    # ... logic removed ...
    
    # Ensure we don't modify the tiers based on hardcoded values
    pass
    # else block removed
    ...
    # 11. Construct Mission Bar (Legacy support + New List)
    mission_bar = None
    community_goals_states = await giveaway_service.get_giveaway_states(db, reviewer.id)
    
    # For legacy frontend support, pick the first one or LIKES
    primary_goal = next((g for g in community_goals_states if g.type == 'LIKES'), community_goals_states[0] if community_goals_states else None)
    
    if primary_goal:
        percent = int((primary_goal.progress / primary_goal.target) * 100) if primary_goal.target > 0 else 0
        mission_bar = schemas.MissionBar(
            status="Active",
            type=primary_goal.type,
            target=primary_goal.target,
            current=primary_goal.progress,
            percent=percent
        )
    ...
    return schemas.LineViewState(
        ...
        giveaway_state=primary_goal, # Legacy
        community_goals=community_goals_states, # New
        reviewer=reviewer
    )
...
@router.post("/test/trigger-goal")
async def trigger_test_goal(
    reviewer_id: int = 1,
    goal_type: str = "SHARES",
    amount: int = 10,
    db: AsyncSession = Depends(get_db)
):
    """
    Temporary endpoint to trigger a community goal update for debugging.
    """
    ...
    return {"status": "triggered", "reviewer_id": reviewer_id, "type": goal_type, "amount": amount}
