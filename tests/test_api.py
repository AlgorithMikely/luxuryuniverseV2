...
def test_unauthorized_access(db_session):
    ...
    response = client.get(f"/{reviewer.id}/queue")
...
def test_reviewer_isolation(db_session):
    ...
    response = client.get(f"/{reviewer1.id}/queue", headers={"Authorization": f"Bearer {token1}"})
    assert response.status_code == 200

    # Accessing other reviewer's queue should be forbidden
    response = client.get(f"/{reviewer2.id}/queue", headers={"Authorization": f"Bearer {token1}"})
