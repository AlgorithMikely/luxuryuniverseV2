...
const Navbar = () => {
  ...
  // Provide a default user object to prevent crashes on initial render
  const displayUser = user || { username: '', avatar: null, roles: [] };

  useEffect(() => {
    const fetchReviewers = async () => {
      if (displayUser.roles.includes("admin")) {
        try {
  ...
  return (
    <nav className="bg-gray-800 text-white p-4 flex justify-between items-center">
      ...
      <div className="relative">
        {/* Only render button if user object is populated */}
